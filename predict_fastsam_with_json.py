...
def run_fastsam_inference(img_path, fastsam_model, device, imgsz=1024, conf=0.4, iou=0.9):
    ...
    try:
        ...
        ann_person=prompt_process.text_prompt("person")
        # 生成边缘掩码（仅内存操作）
        ...
        person_mask=ann_person.astype(np.uint8) * 255
...
def main():
    ...
    for img_idx, img_path in enumerate(tqdm(input_images, desc="推理+优化进度")):
        ...
        if sam_result["status"] == "success" and hisam_result["status"] == "success":
            # 直接从内存获取掩码数组
            sam_person_mask=sam_result["person_mask"]
            ...
            # 保存人物掩码
            person_mask_path = os.path.join(args.output, f"{img_name}_sam_person_mask.png")
            cv2.imwrite(person_mask_path, sam_person_mask)
            # 人物掩码二值化（0和1）
            person_mask_bin = (sam_person_mask > 127).astype(np.uint8)
            add_coco_annotation(coco_data, img_idx + 1, person_mask_bin, 1)  # 类别1: person
            ...
            add_coco_annotation(coco_data, img_idx + 1, text_mask_bin, 2)  # 类别2: text
            ...
            add_coco_annotation(coco_data, img_idx + 1, edge_mask_bin, 4)  # 类别4: edge
