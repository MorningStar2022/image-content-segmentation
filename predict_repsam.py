...
rep_sam_source_dir = os.path.join(project_root, "RepViT-main")
if rep_sam_source_dir not in sys.path:
    sys.path.insert(0, rep_sam_source_dir)

# -------------------------- 导入模型相关模块 --------------------------
from sam.repvit_sam import sam_model_registry, SamAutomaticMaskGenerator, SamPredictor
...
def get_args_parser():
    ...
    # SAM配置
    parser.add_argument("--sam_model_type", type=str, default="repvit", help="SAM模型类型 ['repvit']")
...
def main():
    ...
    # 2. 加载模型
    print("\n🚀 加载模型...")
    rep_sam = sam_model_registry[args.sam_model_type](checkpoint=args.sam_checkpoint)
    rep_sam.to(device=args.device)
    rep_sam.eval()
